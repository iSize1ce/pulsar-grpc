...
// fillOptionalNulls patches the JSON output to include null for proto3 optional
// fields that are not set in the message.
//
// Why this is needed:
// protojson.Marshal with EmitUnpopulated emits zero values for regular fields
// but silently skips "synthetic oneofs" (the internal representation of proto3
// optional fields). Without this fix, the frontend can't distinguish between
// "field was not set" (should show null) and "field doesn't exist."
func fillOptionalNulls(md protoreflect.MessageDescriptor, msg protoreflect.Message, out map[string]any) {
	for i := 0; i < md.Fields().Len(); i++ {
		fd := md.Fields().Get(i)
		name := fd.JSONName()
		od := fd.ContainingOneof()

		// Proto3 optional (synthetic oneof) that's not present → set null
		if od != nil && od.IsSynthetic() && !msg.Has(fd) {
			out[name] = nil
			continue
		}

		// Only recurse into message fields that are actually populated
		if fd.Kind() != protoreflect.MessageKind || !msg.Has(fd) {
			continue
		}

		// Recurse into repeated message fields (each array element)
		// Skip map fields — they use Map(), not List()
		if fd.IsList() {
			list := msg.Get(fd).List()
			arr, _ := out[name].([]any)
			for j := 0; j < list.Len() && j < len(arr); j++ {
				if nested, ok := arr[j].(map[string]any); ok {
					fillOptionalNulls(fd.Message(), list.Get(j).Message(), nested)
				}
			}
			continue
		}

		// Recurse into singular message fields (skip maps)
		if fd.IsMap() {
			continue
		}
		if nested, ok := out[name].(map[string]any); ok {
			fillOptionalNulls(fd.Message(), msg.Get(fd).Message(), nested)
		}
	}
}
